...
import { Search, X, ChevronDown, Minimize2, Maximize2 } from "lucide-react";
...
export default function AirPollutionControl({
  ...
  return (
    <motion.div
      ...
      className="absolute z-50 top-3 left-3 text-sm"
      style={{
        fontFamily: "Inter, system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial",
        background: "rgba(0, 0, 0, 0.6)",
        backdropFilter: "blur(10px)",
        borderRadius: "20px",
        minWidth: minimized ? "auto" : "260px",
        maxWidth: minimized ? "auto" : "300px"
      }}
    >
      <div className="text-white p-4 shadow-2xl">
        {minimized ? (
          <div className="flex items-center gap-3">
            <div className="font-semibold text-base">Air Pollution</div>
            <button
              onClick={() => setMinimized(false)}
              className="bg-gray-800/40 hover:bg-gray-800/60 transition p-1.5 rounded-md"
              title="Maximize"
            >
              <Maximize2 size={16} />
            </button>
